...
class GeminiApiClient:
    ...
    _EXTREME_ASPECT_RATIOS: Set[str] = {"1:4", "4:1", "1:8", "8:1"}
    _EXTREME_ASPECT_RATIO_MODEL_PREFIX = "gemini-3.1-flash-image-preview"
    ...
    def _is_extreme_aspect_ratio_allowed(self, normalized_model: str) -> bool:
        return normalized_model.startswith(self._EXTREME_ASPECT_RATIO_MODEL_PREFIX)

    def _resolve_aspect_ratio_for_model(self, aspect_ratio: Optional[str], normalized_model: str) -> Optional[str]:
        aspect = self._normalize_aspect_ratio(aspect_ratio)
        if not aspect:
            return None
        if aspect in self._EXTREME_ASPECT_RATIOS and not self._is_extreme_aspect_ratio_allowed(normalized_model):
            model_text = normalized_model or "未知模型"
            self.logger.warning(f"模型 {model_text} 不支持比例 {aspect}，已自动回退为 Auto。")
            return None
        return aspect

    def _detect_mime_type(self, data: str) -> str:
        """根据数据内容（URL后缀或Base64头）推断 MIME Type"""
        data_stripped = data.strip()
        if data_stripped.startswith("http"):
            # URL 处理
            guessed_type, _ = mimetypes.guess_type(data_stripped)
            if guessed_type:
                return guessed_type
            # 简单兜底
            lower_url = data_stripped.lower()
            ...
            # PNG: iVBORw
            # WebP: UklGR
            if data_stripped.startswith("/9j/"):
                return "image/jpeg"
            if data_stripped.startswith("UklGR"):
                return "image/webp"
            return "image/png"
    ...
    def create_request_data(
        ...
        for encoded in input_images_b64 or []:
            if not encoded:
                continue
            mime_type = self._detect_mime_type(encoded)
            parts.append({
                "inlineData": {
                    "mimeType": mime_type,
                    "data": encoded,
        ...
        aspect = self._resolve_aspect_ratio_for_model(aspect_ratio, normalized_model)
        if aspect:
            image_config["aspectRatio"] = aspect
        if (
            normalized_model.startswith("gemini-3-pro-image-preview")
            or normalized_model.startswith("gemini-3-pro-image")
            or normalized_model.startswith("gemini-3.1-flash-image-preview")
        ):
    ...
    def _summarize_error_response(self, response: Optional[requests.Response], verbose: bool = False) -> str:
        """
        提取对用户安全的错误摘要。
        Args:
            verbose: 是否返回详细错误信息（用于内部测试或受信任的渠道）。
        """
        if response is None:
            return "无响应内容"
    ...
    def send_request(
        ...
        for attempt in range(1, effective_max_retries + 1):
            ...
            try:
                ...
                truncated = self._summarize_error_response(exc.response, verbose=verbose_error)
